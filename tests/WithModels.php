...
trait WithModels {
     ...
     * @param Collection<array-key, mixed>|array<mixed> $models
     */
    protected static function assertModelsTrashed(array $expected, Collection|array $models): void {
        $actual = (new Collection($models))
            ->filter(static function (mixed $model): bool {
                return $model instanceof Model;
            })
            ->map(static function (Model $model): bool {
                return self::withModelsIsTrashed($model->fresh());
            })
            ->all();
